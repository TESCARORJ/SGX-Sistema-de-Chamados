...
public sealed class MarcarMinhaNotificacaoComoNaoLidaUseCase(
    IRepository<Notificacao> notificacaoRepository,
    IUsuarioContextoAplicacaoService usuarioContextoAplicacaoService,
    IUnitOfWork unitOfWork) : IMarcarMinhaNotificacaoComoNaoLidaUseCase
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (!notificacao.Lida)
        {
            return new AlterarLeituraNotificacaoResponse(notificacao.Id, false, null, false);
        }

        notificacao.MarcarComoNaoLida(usuarioAtual.Login, usuarioAtual.Id);
        await unitOfWork.SaveChangesAsync(cancellationToken);
        return new AlterarLeituraNotificacaoResponse(notificacao.Id, false, null, true);
    }
}
