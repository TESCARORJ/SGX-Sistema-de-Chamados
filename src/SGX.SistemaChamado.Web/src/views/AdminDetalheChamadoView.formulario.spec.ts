...
describe('AdminDetalheChamadoView - respostas do formulario', () => {
  ...
  it('deve manter o detalhe administrativo compativel quando nao houver respostas', () => {
  ...
  it('deve continuar usando apenas o endpoint existente de detalhe administrativo', () => {
    const fonte = lerFonte()

    expect(fonte).toContain('adminService.obterChamadoAdmin(chamadoId)')
    expect(fonte).not.toContain('fetch(')
    expect(fonte).not.toContain('respostas-formulario')
    expect(fonte).not.toContain('adminService.obterRespostasFormulario')
  })
})
